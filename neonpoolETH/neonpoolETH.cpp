// TODO CHANGE FILE PATHS IN CODE ACCORDING TO YOUR ENVIRONMENT. THE REST OF CARBYNE CODE IS PORTABLE, JUST COPY THE PROJECT FOLDER AND RUN PREFERABLY IN CLION
...
int rbf_check, cbf_txpool_lookup, non_block_not_removed=0;
...
//PROBABILISTIC 
bf::counting_bloom_filter TxID_cbfilter_one(make_hasher(k,time(0), true), buckets, 1);
...
int main() {
    ...
    while (entry.is_open() && hour<=400) {   //checking whether the file is open


            if ((hour % (expiry_interval) == 0) ) {
